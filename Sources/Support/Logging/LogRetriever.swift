...
public class LogRetriever {
    ...
    public var apps: [AppLogContainer] {
        get throws {
            ...
            case .none:
                return try getExecutables(url: logsFolder).map { AppLogContainer(bundleIdentifier: $0.bundleIdentifier, executables: [$0]) }
            case .byAppBundleIdentifier(let pathExtension):
                ...
                return try contents
                    .filter { $0.pathExtension == pathExtension }
                    .map { AppLogContainer(bundleIdentifier: $0.deletingPathExtension().lastPathComponent, executables: try getExecutables(url: $0)) }
            }
        }
    }
}
